...
const Sidebar = ({ collapsed, onToggle, currentPage, onPageChange }) => {
  ...
  return (
    ...
    >
      {/* Logo */}
      <div className="p-5 border-slate-200/50 dark:border-slate-700/50 border-b">
      ...
      {/* Navigation I will display Dynamically */}
            ...
            {!collapsed && item.submenu && expandedItems.has(item.id) && (
              <div className="ml-8 mt-2 space-y-1">
                {item.submenu.map((subitem) => {
                  return (
                    <button className="w-full text-left p-2 text-sm text-slate-600 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slage-800/50 rounded-lg transition-all">
                      {subitem.label}
                    </button>
                  );
                })}
